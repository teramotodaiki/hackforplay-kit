...
"\tbeam_late = 20;//くまがビームをだせる時間のかんかく\n"+
"\n"+
"\t//=========================\n"+
"\n"+
"\n"+
...
"\t\tHack.player.beam = [];//生成されるビームの入れ場所\n"+
...
"\t\t\t\tvar clone = new Beam();\n"+
"\t\t\t\tclone.direction = Hack.player.direction;//ビームの向きをくまの向きに合わせる\n"+
"\t\t\t\tclone.x =Hack.player.x+Hack.player.width/2-Hack.player.width*clone.direction;//ビームの出る位置を調整している\n"+
"\t\t\t\tclone.y = Hack.player.y;//ビームの出る位置を調整している\n"+
...
"\t\t\t\t}\n"+
"\n"+
"\t\t\t\tfor(var j = 0; j < Hack.player.beam.length;j++)\n"+
"\t\t\t\t{\n"+
"\t\t\t\t\tif(Hack.player.beam[j].within(Hack.monster[i],20+Hack.monster[i].size))//ビームがモンスターに当たった時\n"+
"\t\t\t\t\t{\n"+
"\t\t\t\t\t\t//モンスターを消す"
"\t\t\t\t\t\tHack.defaultParentNode.removeChild(Hack.monster[i]);\n"+
"\t\t\t\t\t\tgame.rootScene.removeChild(Hack.monster[i]);\n"+
...
"\t\t\t\t\t\t\n"+
"\t\t\t\t\t\t//ビームを消す"
"\t\t\t\t\t\tHack.defaultParentNode.removeChild(Hack.player.beam[j]);\n"+
"\t\t\t\t\t\tgame.rootScene.removeChild(Hack.player.beam[j]);\n"+
"\t\t\t\t\t\tHack.player.beam.splice(j,1);\n"+
"\t\t\t\t\t}\n"+
"\t\t\t\t\telse \n"+
"\t\t\t\t\t{\n"+
"\t\t\t\t\t\tif((Hack.player.beam[j].x > 450+32)||(Hack.player.beam[j].x < 0-32))//ビームが画面外に出た時\n"+
"\t\t\t\t\t\t{\n"+
"\t\t\t\t\t\t\t//ビームを消す"
"\t\t\t\t\t\t\tHack.defaultParentNode.removeChild(Hack.player.beam[j]);\n"+
"\t\t\t\t\t\t\tgame.rootScene.removeChild(Hack.player.beam[j]);\n"+
...
"\t\t\t{\n"+
"\t\t\t\tthis.x += this.speed;\n"+
"\t\t\t\tthis.rotation = 180;//ビームの画像の向きを180度回転している\n"+
"\t\t\t}\n"+
"\t\t\telse\n"+
"\t\t\t{\n"+
"\t\t\t\tthis.x -= this.speed;\n"+
...
"\t//==================================\n";
	...
	game.onload = game.onload || function() {
		...
		Hack.pad = new Pad();
		Hack.pad.moveTo(380,100);
		//Hack.defaultParentNode.addChild(pad);
	...
	game.onenterframe = game.onenterframe || function() {
		...
		if (!Hack.player.isDamaged) {
			for(var i = 0; i < Hack.monster.length;i++)
			{
				...
				for(var j = 0; j < Hack.player.beam.length;j++)
				{
					if(Hack.player.beam[j].within(Hack.monster[i],20+Hack.monster[i].size))
					{
						Hack.defaultParentNode.removeChild(Hack.monster[i]);
						game.rootScene.removeChild(Hack.monster[i]);
						Hack.monster.splice(i,1);
					...
					{
						if((Hack.player.beam[j].x > 450+32)||(Hack.player.beam[j].x < 0-32))
						{
							Hack.defaultParentNode.removeChild(Hack.player.beam[j]);
							game.rootScene.removeChild(Hack.player.beam[j]);
							Hack.player.beam.splice(j,1);	
